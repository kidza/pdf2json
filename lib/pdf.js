...
let PDFJSClass = (function () {
    ...
    cls.prototype.raiseNonRenderingDataReadyEvent = function(data) {
        process.nextTick( () => this.emit("pdfjs_nonRenderingDataReady", data) );
        return data;
    };
    ...
    cls.prototype.getNumberOfPages = function(arrayBuffer) {
        this.pdfDocument = null;

        function getDocumentNumberOfPages(pdfDocument) {
            this.raiseNonRenderingDataReadyEvent({pageNumbers: pdfDocument.pdfInfo.numPages});
            process.nextTick(() => this.raiseNonRenderingDataReadyEvent(null));
        }

        let parameters = {password: '', data: arrayBuffer};
        PDFJS.getDocument(parameters).then(
            pdfDocument => getDocumentNumberOfPages.call(this, pdfDocument),
            error => this.raiseErrorEvent("An error occurred while parsing the PDF: " + error)
        );
    };
	...
	//                Content:pdfPage.getTextContent(),
                    ...
                    viewPortHeight: pageParser.viewport.height,
                    viewPortWidth: pageParser.viewport.width,
                    viewPortScale: pageParser.viewport.scale,
	            };
